...
public class ParameterServiceImpl extends ServiceImpl<ParameterMapper, Parameter> implements IParameterService {
    ...
    @Override
    public List<ParameterVO> goodsTypes() {
        return this.list(Wrappers.lambdaQuery(Parameter.class).eq(Parameter::getGroupName, ParameterGroup.GOODS_TYPE).orderByAsc(Parameter::getCode)).stream().map(ParameterVO::new).collect(Collectors.toList());
    }
}
